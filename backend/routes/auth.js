...
const {
  ...
  getRegulationVersionsByCode,
  submitRegulationVersion,
} = require("../controllers/regulation");
...
router.get("/regulations", async (req, res) => {
  try {
    ...
    const grouped = regs.reduce((acc, curr) => {
      ...
      const hodName = curr.hod && curr.hod.name ? curr.hod.name : "Not Assigned";
      acc[code].push({
...
router.put(
  ...
  protect,
  authorize("hod"),
  (req, res) => submitRegulationVersion(req, res)
);

router.post(
  "/hod/regulations/save-draft",
  protect,
  authorize("hod"),
  saveRegulationDraft
);
