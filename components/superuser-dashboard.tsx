...
export default function AdminDashboard({ user }: AdminDashboardProps) {
  ...
  const [showCreateRegDialog, setShowCreateRegDialog] = useState(false);
  ...
  const fetchRegulations = useCallback(async () => {
    try {
      ...
      if (data && typeof data === "object" && !Array.isArray(data)) {
        const normalized = Object.entries(data).reduce((acc, [code, list]) => {
          acc[code] = Array.isArray(list) ? list : [];
          return acc;
      ...
      setRegulations({});
      setRegulationsError(error instanceof Error ? error.message : "Failed to load regulations");
    }
  }, []);
  ...
  const handleDeleteRegulation = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Failed to delete regulation");
    } finally {
  ...
  const AddRegulationButton = () => (
    <Button
      className="bg-purple-600 hover:bg-purple-700 text-white"
      onClick={() => {
        setNewRegulationCode("");
        setShowCreateRegDialog(true);
      }}
    >
      + Add Regulation
    </Button>
          ...
          <div className="space-y-6 animate-fade-in">
            {regulationsError && (
              <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                Failed to load regulations: {regulationsError}
              </div>
            )}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <Card className="hover-lift">
                ...
                <CardContent>
                  {(() => {
                    const latestRegulationCode = Object.keys(regulations).sort().reverse()[0];
                    const latestRegulationItems = latestRegulationCode ? regulations[latestRegulationCode] ?? [] : [];
              ...
              <div className="space-y-4 animate-fade-in">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="text-muted-foreground">Click a regulation to review department-wise curriculum submissions</p>
                  </div>
                  <AddRegulationButton />
                </div>
              ...
              <Card className="animate-slide-up shadow-lg border border-purple-300">
                <CardHeader className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant="destructive"
                      onClick={() => setShowDeleteRegDialog(true)}
  ...
  return (
    <>
      ...
      <Dialog
        open={showCreateRegDialog}
        onOpenChange={(open) => {
          setShowCreateRegDialog(open);
          if (!open) setNewRegulationCode("");
        }}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Provide the regulation code (e.g., R24). A placeholder entry will be created for each department.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <Input
            ...
            <Button
              ...
              disabled={creatingRegulation}
            >
              {creatingRegulation ? "Creating..." : "Create Regulation"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>

      <Dialog open={showDeleteRegDialog} onOpenChange={setShowDeleteRegDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-red-600">Remove Regulation</DialogTitle>
            <DialogDescription>
              This will delete regulation "{selectedRegulation}" and all associated department entries. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" onClick={() => setShowDeleteRegDialog(false)}>
              Cancel
            </Button>
            <Button
              variant="destructive"
            ...
            >
              {isDeletingRegulation ? "Removing..." : "Delete"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
