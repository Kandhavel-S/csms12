...
interface SelectedRegulationMeta {
  _id?: string;
...
interface CreateCurriculumProps {
  ...
  selectedRegulation?: SelectedRegulationMeta | null;
  onDraftSaved?: () => void;
  onSubmitted?: () => void;
  resetSignal?: number;
  onFormChange?: () => void;
...
interface CourseCounts {
  ...
  semester7: number;
  semester8: number;
  openElective1: number;
  openElective2: number;
  mandatoryCourse1: number;
  mandatoryCourse2: number;
}

const buildDefaultFormFields = (): FormFields => ({
  degree: '',
  branchName: '',
  regulation: '',
...
const cloneFormFields = (data: FormFields): FormFields => JSON.parse(JSON.stringify(data));

const stripFileReferences = (payload: any): any => {
  if (Array.isArray(payload)) {
    return payload.map((entry) => stripFileReferences(entry));
  }
  ...
  if (payload && typeof payload === 'object') {
    return Object.entries(payload).reduce((acc, [key, value]) => {
      if (key === 'syllabusFile') {
        ...
        acc[key] = stripFileReferences(value);
      }
      return acc;
...
const serializeFormFields = (fields: FormFields): FormFields => stripFileReferences(fields) as FormFields;

const buildDefaultCourseCounts = (): CourseCounts => ({
  hsmc: 1,
  bsc: 1,
  esc: 1,
  pcc: 1,
...
const deriveCourseCounts = (fields: FormFields): CourseCounts => ({
  hsmc: Math.max(fields.hsmcCourses.length, 1),
  bsc: Math.max(fields.bscCourses.length, 1),
  esc: Math.max(fields.escCourses.length, 1),
  pcc: Math.max(fields.pccCourses.length, 1),
  pec: Math.max(fields.pecCourses.length, 1),
  oec: Math.max(fields.oecCourses.length, 1),
  eec: Math.max(fields.eecCourses.length, 1),
  mc: Math.max(fields.mcCourses.length, 1),
  semester1: Math.max(fields.semester1Courses.length, 1),
  semester2: Math.max(fields.semester2Courses.length, 1),
  semester3: Math.max(fields.semester3Courses.length, 1),
  semester4: Math.max(fields.semester4Courses.length, 1),
  semester5: Math.max(fields.semester5Courses.length, 1),
  semester6: Math.max(fields.semester6Courses.length, 1),
  semester7: Math.max(fields.semester7Courses.length, 1),
  semester8: Math.max(fields.semester8Courses.length, 1),
  openElective1: Math.max(fields.openElectiveTables[0]?.courses.length ?? 0, 1),
  openElective2: Math.max(fields.openElectiveTables[1]?.courses.length ?? 0, 1),
  mandatoryCourse1: Math.max(fields.mandatoryCourseTables[0]?.courses.length ?? 0, 1),
  mandatoryCourse2: Math.max(fields.mandatoryCourseTables[1]?.courses.length ?? 0, 1),
});
...
const CreateCurriculum: React.FC<CreateCurriculumProps> = ({
  user,
  selectedRegulation,
  onDraftSaved,
  onSubmitted,
  ...
  const handleDownloadMainAndMergedSyllabi = async () => {
    ...
    try {
      ...
      if (syllabusBuffers.length > 0) {
        ...
        try {
          const mergerUrl = process.env.NEXT_PUBLIC_MERGER_URL || 'https://csmspy.onrender.com/merge-curriculum-syllabi';
  ...
  const handleAutofill = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://csms-x9aw.onrender.com/api/auth/get-subjects?${params.toString()}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      try {
        ...
        const verticalsRes = await fetch(
          `https://csms-x9aw.onrender.com/api/auth/verticals?${verticalsParams.toString()}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const savedTimestamp = data?.regulation?.savedAt || new Date().toISOString();
      setDraftSaveFeedback(`Draft saved at ${formatDisplayDate(savedTimestamp)}`);
      onDraftSaved?.();
    } catch (error: any) {
  ...
  const handleManualUpload = async () => {
    ...
    if (!formFields.regulation.trim()) {
      toast.error("Please provide the regulation code before uploading.");
      return;
    }

    const departmentName = formFields.branchName || user.department || '';
    if (!departmentName.trim()) {
      toast.error("Please provide your department before uploading.");
      return;
    }
